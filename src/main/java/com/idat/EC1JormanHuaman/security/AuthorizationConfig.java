...
public class AuthorizationConfig extends AuthorizationServerConfigurerAdapter {
	...
	public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
		...
		clients.inMemory()
			.withClient("ClienteA")
			.secret(new BCryptPasswordEncoder().encode("ClienteA"))
			.authorizedGrantTypes("USER", "authorization_code", "refresh_token", "implicit")
			.scopes("read", "write", "trust")
			.accessTokenValiditySeconds(1*60*60)
			.refreshTokenValiditySeconds(5*60*60);
		clients.inMemory()
		.withClient("AlexisSarabia")
		.secret(new BCryptPasswordEncoder().encode("AlexisSarabia"))
		.authorizedGrantTypes("USER", "authorization_code", "refresh_token", "implicit")
		.scopes("read", "write", "trust")
		.accessTokenValiditySeconds(1*60*60)
		.refreshTokenValiditySeconds(5*60*60);
		clients.inMemory()
		.withClient("JormanHS")
		.secret(new BCryptPasswordEncoder().encode("JormanHS"))
		.authorizedGrantTypes("USER", "authorization_code", "refresh_token", "implicit")
		.scopes("read", "write", "trust")
		.accessTokenValiditySeconds(1*60*60)
		.refreshTokenValiditySeconds(5*60*60);
	...
	public void configure(AuthorizationServerEndpointsConfigurer endpoints) throws Exception {
		...
		tokenChain.setTokenEnhancers(Arrays.asList(  new TokenEnhancer() {
			...
			public OAuth2AccessToken enhance(OAuth2AccessToken accessToken, OAuth2Authentication authentication) {
				...
				informacionAdicional.put("Cita", "Hospital");
